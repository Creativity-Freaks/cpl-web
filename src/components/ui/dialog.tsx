...
>(({ className, children, "aria-describedby": ariaDescribedBy, ...props }, ref) => {
  ...
  React.useEffect(() => {
    if (contentRef.current) {
      const t = contentRef.current.querySelector('[data-dialog-title]');
      setHasTitle(!!t);
    }
  }, [children]);
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        aria-describedby={ariaDescribedBy === undefined ? undefined : ariaDescribedBy}
        {...props}
      >
        {!hasTitle && (
          <span className="sr-only" data-fallback-title data-dialog-title>
            Dialog
          </span>
        )}
        {children}
