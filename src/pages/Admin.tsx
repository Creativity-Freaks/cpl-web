...
const Card = ({ children, className = '', ...p }: any) => (
  <div className={`bg-white shadow-lg rounded-xl ${className}`} {...p}>{children}</div>
);
const CardContent = ({ children, className = '', ...p }: any) => (
  <div className={`p-6 ${className}`} {...p}>{children}</div>
);
const Button = ({ children, variant = 'default', className = '', size = '', ...p }: any) => {
  const v = {
    default: 'bg-blue-600 hover:bg-blue-700 text-white',
    outline: 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50',
  };
  const s = size === 'sm' ? 'px-3 py-1 text-sm' : 'px-8 py-3';
  return <button className={`rounded-lg font-medium ${v[variant]} ${s} ${className}`} {...p}>{children}</button>;
};
const Input = (p: any) => <input className="w-full p-4 border border-gray-300 rounded-lg" {...p} />;
const Label = ({ children, ...p }: any) => <label className="block font-medium mb-2" {...p}>{children}</label>;
const Select = (p: any) => <select className="w-full p-4 border rounded-lg" {...p}>{p.children}</select>;
const Footer = () => <footer className="h-12 flex items-center justify-center text-xs bg-white border-t">© CPL Admin</footer>;
// --- MAIN APP ---
const Admin: React.FC = () => {
  ...
  const writeSoldBlacklist = useCallback((list: string[]) => {
    const key = soldBlacklistKey(auctionTournamentId);
    if (!key) return;
  ...
  // --- FETCH TEAMS ---
  const fetchTeams = useCallback(async (tournamentId: number, silent = false) => {
  ...
  // --- FETCH MATCHES ---
  const fetchMatches = useCallback(async (tournamentId: number, silent = false) => {
    if (!tournamentId) return;
    try {
  ...
  const fetchConfirmedAuctionPlayers = useCallback(async (tournamentId: number): Promise<any[]> => {
    if (!tournamentId) return [];
    try {
  ...
  const loadAuctionData = useCallback(async (silent = false) => {
    ...
    try {
      // Fetch all players (full details) - Extract from data.all_player_list
      const allData = await api.get("/api/v1/adminall/players");
      allPlayersList = Array.isArray(allData?.all_player_list) ? allData.all_player_list : [];
  ...
  }, [auctionTournamentId, fetchConfirmedAuctionPlayers, visibleCount, visibleCountConfirmed]);
  // --- FETCH LIVE PLAYERS ---
  const fetchLivePlayers = useCallback(async (silent = false) => {
    if (!auctionTournamentId) {
      setLivePlayers([]);
      return;
    }
    ...
    try {
      ...
      // Fetch all players for full details to merge accurate categories - Extract from data.all_player_list
      const allData = await api.get("/api/v1/adminall/players");
  ...
  }, [auctionTournamentId, readSoldBlacklist]);
  ...
  }, [allPlayers, displayedPlayers.length, visibleCount]);
  const handleScrollConfirmed = useCallback((e: React.UIEvent<HTMLDivElement>) => {
    const { scrollTop, scrollHeight, clientHeight } = e.currentTarget;
  ...
  }, [confirmedAuctionPlayers.length, displayedConfirmedPlayers.length, visibleCountConfirmed]);
  // --- TOGGLE PLAYER SELECTION (Only highlights, for pending) ---
  const togglePlayerSelection = useCallback((player: any) => {
  ...
  const handleSelectPlayersForAuction = useCallback(async () => {
  ...
  }, [auctionTournamentId, pendingSelectedPlayers, fetchLivePlayers, visibleCountConfirmed, confirmedAuctionPlayers.length]);
  // --- REMOVE PLAYER FROM AUCTION ---
  const handleRemovePlayer = useCallback(async (auctionPlayerId: number) => {
    if (!auctionPlayerId) {
  ...
  }, [loadAuctionData, fetchLivePlayers]);
  // --- CREATE TEAM ---
  ...
  }, [createTeamTournamentId, newTeam, selectedTournamentId, fetchTeams]);
  // --- CREATE MATCH ---
  ...
  }, [selectedMatchTournamentId, newMatch, fetchMatches]);
  // --- UPDATE MATCH STATUS ---
  const updateMatchStatus = useCallback(async (id: number, status: string) => {
  ...
  }, [matches, selectedMatchTournamentId, fetchMatches]);
  // --- CREATE TOURNAMENT ---
  ...
  }, [selectedTournamentId, selectedMatchTournamentId, auctionTournamentId, fetchTournaments, fetchTeams, fetchMatches, loadAuctionData, fetchLivePlayers, readSoldBlacklist, writeSoldBlacklist]);
  ...
  // Automatic background refresh for all sections - Increased interval to 30 seconds to reduce flickering
  // Refresh every 30 seconds when section is active, but don't show loading states
  useEffect(() => {
    ...
    }, 30000); // Increased to 30 seconds to minimize flickering
   
    return () => clearInterval(refreshInterval);
  }, [activeSection, isAuthenticated, selectedTournamentId, selectedMatchTournamentId, auctionTournamentId, fetchTournaments, fetchTeams, fetchMatches, loadAuctionData, fetchLivePlayers]);
  ...
  const changeLiveAmount = useCallback((delta: number) => {
  ...
  const normalizeTeamStats = useCallback((stats: any) => {
                      ...
                      <div className="space-y-2">
                        {displayedPlayers.map(player => (
                          ...
                          >
                            ...
                            {/* Player Image - Using the image helper function */}
                            <img
                      ...
                      {confirmedAuctionPlayers.map(player => (
                        ...
                        >
                          <div className="flex items-center gap-3 flex-1">
                            {/* Player Image in Confirmed Section */}
                            <img
                              src={getPlayerImageUrl(player.photo_url)}
                              alt={player.name}
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-3 max-h-80 overflow-y-auto">
                      {updatedPlayers.map(player => (
                        ...
                        >
                          {/* Player Image in Updated Players Section */}
                          <img
                            src={getPlayerImageUrl(player.photo_url)}
                            alt={player.name}
                            className="w-12 h-12 rounded-full object-cover border border-gray-300"
